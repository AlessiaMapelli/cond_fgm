...
# Compare the graphs
##################
g_true <- graph_from_adjacency_matrix(adj_matrix_g, mode = "undirected", diag = F, weighted=F)
g_true

layout = layout_in_circle(g_true)
plot(g_true,layout= layout, edge.arrow.size=1, vertex.color="lightgreen", vertex.size=10, 
     
     vertex.frame.color="lightgreen", vertex.label.color="black", 
     
     vertex.label.cex=1, vertex.label.dist=0, edge.curved=0.2, edge.color= "gray", edge.width =E(g_true)$weight, margin=c(0,0,0,0)) 


g_est <- graph_from_adjacency_matrix(G.mat.g.sim , mode = "undirected", diag = F, weighted=F)
g_est

layout = layout_in_circle(g_est)
plot(g_est,layout= layout, edge.arrow.size=1, vertex.color="lightskyblue1", vertex.size=10, 
     
     vertex.frame.color="lightskyblue1", vertex.label.color="black", 
     
     vertex.label.cex=1, vertex.label.dist=0, edge.curved=0.2, edge.color= "gray", edge.width =E(g_est)$weight, margin=c(0,0,0,0)) 
