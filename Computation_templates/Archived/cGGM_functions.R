...
GGReg_mean_estimation <-
  ...
  if (is.null(covariates)) {
    ...
    # X = scale(x, center = TRUE, scale = TRUE) try and remove the scaling of the outcome
    X=x
    k <- ncol(C)
    ...
    if (verbose) {
      cat("Initialized parallelization \n ")
    }
    results <- foreach(i = 1:p, .combine = 'rbind', .packages = 'glmnet') %dopar% {
      set.seed(2024)
      Y <- matrix(X[, i], ncol = 1)
      gamma <- matrix(0, k, 1)
      if (k == 2){
        ...
        Cov_effect_row <- as.vector(coef(mod))
      } else {
      if (is.null(lambda_mean)) {
        mod <- cv.glmnet(x = C, y = Y, parallel=TRUE)
        if (lambda_mean_type == "min"){
          gamma <- coef(mod, s = "lambda.min")[, 1]
        } else {
          gamma <- coef(mod, s = "lambda.1se")[, 1] 
        } 
        res.glm <- Y - cbind(1, C) %*% gamma
        Cov_effect_row <- as.vector(gamma)
      } else {
      ...
      list(res.glm = as.vector(res.glm), Cov_effect_row = Cov_effect_row)
      }
    
    stopCluster(cl)
    if (verbose) {
      cat("Done with parallelization \n ")
    }

    for (i in 1:p) {
      z[, i] <- results[i,1][[1]]
      Cov_effect[i, ] <- results[i,2][[1]]
    }
  }
...
GGReg_stable_mean_estimation <-
  ...
  if (is.null(covariates)) {
    ...
    results <- foreach(i = 1:p, .combine = 'rbind', .packages = c('glmnet','stabs')) %dopar% {
    set.seed(2024)
    Y <- matrix(X[, i], ncol = 1)
    gamma <- matrix(0, k, 1)
    rownames(gamma) <- colnames(Cov_effect)
    if (k == 2){
        data <- data.frame(x = C, y = Y)
        colnames(data) <- c("x","y")
        mod <- lm(y ~ x, data)
    ...
    stab.glmnet <- stabsel(x = C, y = Y,
                          fitfun = glmnet.lasso, cutoff = 0.95,
                          q=2)
    glm.model <- glm.fit(x=cbind(rep(1,nrow(C)),C[,names(stab.glmnet$selected)]), y=Y, family=gaussian())
    gamma[c("(Intercept)", names(stab.glmnet$selected)),1] <- as.vector(glm.model$coefficients)
    res.glm <- glm.model$residuals
    Cov_effect_row <- as.vector(gamma)}
    list(res.glm = as.vector(res.glm), Cov_effect_row = Cov_effect_row)
  }
